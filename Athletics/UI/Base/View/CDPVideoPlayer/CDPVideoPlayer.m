...
-(void)createTopBar{
    if (_topBar==nil) {
        ...
        //标题
        _titleLabel=[[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_backButton.frame) + 5,_topBar.frame.origin.y,_topBar.frame.size.width-CGRectGetWidth(_backButton.frame) * 2 - 10,CDPTOPHEIGHT(NO))];
...
-(void)switchClick{
    ...
    if ([_delegate respondsToSelector:@selector(switchSizeClickToFullScreen:)]) {
        [_delegate switchSizeClickToFullScreen:_isFullScreen];
    }
}
//拖动slider时,改变当前播放时间
-(void)sliderChange{
